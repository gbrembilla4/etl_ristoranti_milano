"""
Script di pulizia semplificato per file JSON di piatti
...
# === LOGGING ===
log_path = os.path.join(os.path.dirname(__file__), "..", "..", "logs", "etl", "pulizia_piatti_con_stime.log")
...
def main():
    ...
    # === Carica JSON ===
    try:
        piatti_file = os.path.join(os.path.dirname(__file__), "..", "..", CONFIG["file_paths"]["clean_dishes_llama"])
    ...
    # === Salva nuovo JSON ===
    output_path = piatti_file.replace(".json", "_no_bibite.json")
    with open(output_path, "w", encoding="utf-8") as f_out:
        json.dump(piatti_finali, f_out, ensure_ascii=False, indent=2)
    logger.info(f"Salvati {len(piatti_finali)} piatti nel file: {output_path}")
